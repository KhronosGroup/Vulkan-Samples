...
// Closest-hit shader for OBJECT_NORMAL (type 0) - diffuse surfaces with textures
...
void main()
{
    ...
    const uint objectType = 0; // OBJECT_NORMAL
    ...
    // Diffuse texture lookup with additional computation to create divergence
    if (imageOffset < 26) {
        vec4 tex_value = textureLod(textures[nonuniformEXT(imageOffset)], texcoord, 0);
        
        // Additional diffuse shading computation (creates work divergence from other shaders)
        float diffuse = max(dot(worldNormal, normalize(vec3(0, -1, 0))), 0.0);
        float ambient = 0.3;
