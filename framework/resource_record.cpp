...
size_t ResourceRecord::register_pipeline_layout(const std::vector<ShaderModule *> &shader_modules)
{
	...
	std::transform(shader_modules.begin(), shader_modules.end(), shader_indices.begin(), [this](ShaderModule *shader_module) {
...
size_t ResourceRecord::register_render_pass(const std::vector<vkb::rendering::AttachmentC> &attachments,
