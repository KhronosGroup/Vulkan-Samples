...
bool Platform::initialize(const std::vector<Plugin *> &plugins = {})
{
	...
	create_window(default_window_properties);
	...
	while (!window->should_close())
	{
		try
		{
			...
			if (app_requested())
			{
				...
				// TODO: Setting the framerate to 60 fps isn't a long term fix. There should be a way of disabling features of app from running on the first frame possibly a skip_frame flag in Application::update()
...
{
	auto context = std::make_unique<RenderContext>(device, surface, window->get_extent().width, window->get_extent().height);
...
void Platform::set_default_window_properties(const Window::OptionalProperties &properties)
{
	default_window_properties.title = properties.title.has_value() ? properties.title.value() : default_window_properties.title;
	default_window_properties.mode  = properties.mode.has_value() ? properties.mode.value() : default_window_properties.mode;

	// Only override if resizable is currently enabled
	if (default_window_properties.resizable != false && properties.resizable.has_value())
	{
		default_window_properties.resizable = properties.resizable.value();
	}

	// Only override when in default state
	if (default_window_properties.vsync == Window::Vsync::Default && properties.vsync.has_value())
	{
		default_window_properties.vsync = properties.vsync.value();
	}

	default_window_properties.extent.width  = properties.extent.width.has_value() ? properties.extent.width.value() : default_window_properties.extent.width;
	default_window_properties.extent.height = properties.extent.height.has_value() ? properties.extent.height.value() : default_window_properties.extent.height;
}
...
{
	auto extent = Window::Extent{std::max<uint32_t>(width, 420), std::max<uint32_t>(height, 320)};
	if (window)
	{
