...
void cursor_position_callback(GLFWwindow *window, double xpos, double ypos)
{
	if (auto *platform = reinterpret_cast<Platform *>(glfwGetWindowUserPointer(window)))
	{
		platform->input_event(MouseButtonInputEvent{MouseButton::Unknown, MouseAction::Move, static_cast<float>(xpos), static_cast<float>(ypos)});
	}
}
...
void mouse_button_callback(GLFWwindow *window, int button, int action, int /*mods*/)
{
	...
	if (auto *platform = reinterpret_cast<Platform *>(glfwGetWindowUserPointer(window)))
	{
		...
		platform->input_event(MouseButtonInputEvent{translate_mouse_button(button), mouse_action, static_cast<float>(xpos), static_cast<float>(ypos)});
	}
}
}        // namespace
