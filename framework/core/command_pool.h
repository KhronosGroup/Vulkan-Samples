...
class CommandPool
{
  ...
  private:
	vkb::core::CommandBufferCpp &request_command_buffer_impl(vk::CommandBufferLevel level);
...
vkb::core::CommandBuffer<bindingType> &CommandPool<bindingType>::request_command_buffer(CommandBufferLevelType level)
{
	if constexpr (bindingType == vkb::BindingType::Cpp)
	{
		return request_command_buffer_impl(level);
	}
	else
	{
		return reinterpret_cast<vkb::core::CommandBufferC &>(request_command_buffer_impl(static_cast<vk::CommandBufferLevel>(level)));
	}
}

template <vkb::BindingType bindingType>
inline vkb::core::CommandBufferCpp &CommandPool<bindingType>::request_command_buffer_impl(vk::CommandBufferLevel level)
{
	if (level == vk::CommandBufferLevel::ePrimary)
	{
		...
		primary_command_buffers.emplace_back(*reinterpret_cast<vkb::core::CommandPoolCpp *>(this), level);
	...
	else
	{
		...
		secondary_command_buffers.emplace_back(*reinterpret_cast<vkb::core::CommandPoolCpp *>(this), level);
