...
class HPPPipelineLayout
{
  public:
	...
	std::vector<vkb::core::HPPShaderResource>
	    get_resources(const vkb::core::HPPShaderResourceType &type  = vkb::core::HPPShaderResourceType::All,
  ...
  private:
	...
	std::vector<vkb::core::HPPShaderModule *>
	    shader_modules;        // The shader modules that this pipeline layout uses
	std::unordered_map<std::string, vkb::core::HPPShaderResource>
	    shader_resources;        // The shader resources that this pipeline layout uses, indexed by their name
	std::unordered_map<uint32_t, std::vector<vkb::core::HPPShaderResource>>
	    shader_sets;        // A map of each set and the resources it owns used by the pipeline layout
	std::vector<vkb::core::HPPDescriptorSetLayout *>
	    descriptor_set_layouts;        // The different descriptor set layouts for this pipeline layout
};
}        // namespace core
}        // namespace vkb
