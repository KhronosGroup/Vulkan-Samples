...
void DescriptorSet::update(const std::vector<uint32_t> &bindings_to_update)
{
	...
	if (!write_operations.empty())
	{
		vkUpdateDescriptorSets(
		    device.get_handle(), to_u32(write_operations.size()), write_operations.data(), 0, nullptr);
	}
...
{
	vkUpdateDescriptorSets(
	    device.get_handle(), to_u32(write_descriptor_sets.size()), write_descriptor_sets.data(), 0, nullptr);
}
