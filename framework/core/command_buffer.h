    ...
    : public vkb::core::VulkanResource<bindingType, typename std::conditional<bindingType == vkb::BindingType::Cpp, vk::CommandBuffer, VkCommandBuffer>::type>
{
  ...
  public:
	...
	void
	    image_memory_barrier(vkb::rendering::RenderTarget<bindingType> &render_target, uint32_t view_index, ImageMemoryBarrierType const &memory_barrier) const;
	void next_subpass();
...
inline void CommandBuffer<bindingType>::begin_impl(vk::CommandBufferUsageFlags flags, vkb::core::CommandBuffer<bindingType> *primary_cmd_buf)
{
	if (level == vk::CommandBufferLevel::eSecondary)
	{
		...
		return begin_impl(
		    flags, primary_cmd_buf->current_render_pass, primary_cmd_buf->current_framebuffer, primary_cmd_buf->pipeline_state.get_subpass_index());
	}
	else
	{
...
template <vkb::BindingType bindingType>
inline void
    CommandBuffer<bindingType>::bind_image(ImageViewType const &image_view, SamplerType const &sampler, uint32_t set, uint32_t binding, uint32_t array_element)
{
	if constexpr (bindingType == vkb::BindingType::Cpp)
	{
...
template <vkb::BindingType bindingType>
inline void
    CommandBuffer<bindingType>::draw_indexed_indirect(vkb::core::Buffer<bindingType> const &buffer, DeviceSizeType offset, uint32_t draw_count, uint32_t stride)
{
	flush(vk::PipelineBindPoint::eGraphics);
	if constexpr (bindingType == vkb::BindingType::Cpp)
	{
    ...
    CommandBuffer<bindingType>::execute_commands_impl(std::vector<std::shared_ptr<vkb::core::CommandBuffer<vkb::BindingType::Cpp>>> &secondary_command_buffers)
{
	...
	std::transform(secondary_command_buffers.begin(), secondary_command_buffers.end(), sec_cmd_buf_handles.begin(), [](auto const &sec_cmd_buf) {
		return sec_cmd_buf->get_handle();
	});
...
inline void CommandBuffer<bindingType>::flush_descriptor_state_impl(vk::PipelineBindPoint pipeline_bind_point)
{
	...
	if (resource_binding_state.is_dirty() || !update_descriptor_sets.empty())
	{
		...
		for (auto &resource_set_it : resource_binding_state.get_resource_sets())
		{
			...
			// Bind descriptor set
			this->get_resource().bindDescriptorSets(
			    pipeline_bind_point, pipeline_layout.get_handle(), descriptor_set_id, descriptor_set_handle, dynamic_offsets);
		}
	}
}
