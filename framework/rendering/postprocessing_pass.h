...
class PostProcessingPass : public PostProcessingPassBase
{
  public:
	...
	/**
	 * @brief Set the render target to output to.
	 * 				If set, this pass will output to the given render target instead of the one passed to draw().
	 *
	 * @param new_render_target the render target to use as an output target
	 * @return Self& a reference to the calling object
	 */
