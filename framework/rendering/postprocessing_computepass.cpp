...
void PostProcessingComputePass::prepare(vkb::core::CommandBufferC &command_buffer, vkb::rendering::RenderTargetC &default_render_target)
{
...
void PostProcessingComputePass::transition_images(vkb::core::CommandBufferC &command_buffer, vkb::rendering::RenderTargetC &default_render_target)
{
	...
	for (const auto &storage : storage_images)
	{
		if (const uint32_t *attachment = storage.second.get_target_attachment())
		{
			...
			auto resource = std::find_if(pipeline_layout.get_resources().begin(), pipeline_layout.get_resources().end(), [&storage](const auto &res) {
...
void PostProcessingComputePass::draw(vkb::core::CommandBufferC &command_buffer, vkb::rendering::RenderTargetC &default_render_target)
{
