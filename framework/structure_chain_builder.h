...
class StructureChainBuilder
{
  public:
	...
	template <typename T>
	T &add_chain_data(T const &data_to_add = {});        // Adds data to the structure chain builder that is not part of the structure chain itself, but is used
	                                                     // by the structures in the chain (e.g. pointed to by a member of a struct in the structure chain)
  ...
  private:
	...
	std::vector<std::unique_ptr<std::any>> chain_data;        // data used with the structure chain, stored separately to ensure correct destruction order (the
	                                                          // structs in the structure chain may contain pointers to data owned by the chain_data)
};
...
{
#if !defined(NDEBUG)
	auto it = std::ranges::find_if(structure_chain, [](auto const &chain_element) { return std::any_cast<StructType>(chain_element.get()) != nullptr; });
	assert(it == structure_chain.end() ||
	       StructType::allowDuplicate);        // If the struct type already exists in the structure chain, it must allow duplicates
#endif
...
inline StructType const *StructureChainBuilder<bindingType, AnchorStructType>::get_struct_impl(size_t skip) const
{
	...
	{
		it = std::find_if(
		    std::next(it), structure_chain.end(), [](auto const &chain_element) { return std::any_cast<StructType>(chain_element.get()) != nullptr; });
	}
