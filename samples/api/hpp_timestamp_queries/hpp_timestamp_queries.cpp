...
void HPPTimestampQueries::on_update_ui_overlay(vkb::Drawer &drawer)
{
	...
	if (drawer.header("timing"))
	{
		...
		drawer.text("Pass 2: %s %.3f ms", (bloom.enabled ? "First bloom pass" : "Scene display"),
...
{
	{
		...
		offscreen.sampler = vkb::common::create_sampler(get_device().get_gpu().get_handle(), get_device().get_handle(), offscreen.color[0].format,
	...
	// Bloom separable filter pass
	{
		...
		filter_pass.sampler = vkb::common::create_sampler(get_device().get_gpu().get_handle(), get_device().get_handle(), filter_pass.color.format,
		                                                  vk::Filter::eNearest, vk::SamplerAddressMode::eClampToEdge, 1.0f, 1.0f);
	}
}
