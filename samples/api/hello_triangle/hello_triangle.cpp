 ...
 */
bool HelloTriangle::validate_extensions(const std::vector<const char *> &required, const std::vector<VkExtensionProperties> &available)
{
...
#	pragma error Platform not supported
	...
	VkApplicationInfo app{
	    .sType = VK_STRUCTURE_TYPE_APPLICATION_INFO, .pApplicationName = "Hello Triangle", .pEngineName = "Vulkan Samples", .apiVersion = VK_API_VERSION_1_1};
...
void HelloTriangle::init_device()
{
	...
	VmaVulkanFunctions vma_vulkan_func{.vkGetInstanceProcAddr = vkGetInstanceProcAddr, .vkGetDeviceProcAddr = vkGetDeviceProcAddr};

	VmaAllocatorCreateInfo allocator_info{
	    .physicalDevice = context.gpu, .device = context.device, .pVulkanFunctions = &vma_vulkan_func, .instance = context.instance};
...
void HelloTriangle::init_vertex_buffer()
{
	...
	VmaAllocationCreateInfo buffer_alloc_ci{.flags         = VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT | VMA_ALLOCATION_CREATE_MAPPED_BIT,
...
void HelloTriangle::init_pipeline()
{
	...
	VkVertexInputBindingDescription binding_description{.binding = 0, .stride = sizeof(Vertex), .inputRate = VK_VERTEX_INPUT_RATE_VERTEX};
...
void HelloTriangle::render_triangle(uint32_t swapchain_index)
{
	...
	VkRect2D scissor{.extent = {.width = context.swapchain_dimensions.width, .height = context.swapchain_dimensions.height}};
