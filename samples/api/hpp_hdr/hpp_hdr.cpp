...
vk::Pipeline HPPHDR::create_bloom_pipeline(uint32_t direction)
{
	...
	return vkb::common::create_graphics_pipeline(get_device().get_handle(), pipeline_cache, shader_stages, {}, vk::PrimitiveTopology::eTriangleList, 0,
	                                             vk::PolygonMode::eFill, vk::CullModeFlagBits::eFront, vk::FrontFace::eCounterClockwise,
	                                             {blend_attachment_state}, depth_stencil_state, bloom.pipeline_layout,
...
vk::Pipeline HPPHDR::create_composition_pipeline()
{
	...
	return vkb::common::create_graphics_pipeline(get_device().get_handle(), pipeline_cache, shader_stages, {}, vk::PrimitiveTopology::eTriangleList, 0,
	                                             vk::PolygonMode::eFill, vk::CullModeFlagBits::eFront, vk::FrontFace::eCounterClockwise,
	                                             {blend_attachment_state}, depth_stencil_state, composition.pipeline_layout, render_pass);
}
...
vk::Pipeline HPPHDR::create_models_pipeline(uint32_t shaderType, vk::CullModeFlagBits cullMode, bool depthTestAndWrite)
{
	...
	return vkb::common::create_graphics_pipeline(get_device().get_handle(), pipeline_cache, shader_stages, vertex_input_state,
	                                             vk::PrimitiveTopology::eTriangleList, 0, vk::PolygonMode::eFill, cullMode, vk::FrontFace::eCounterClockwise,
	                                             blend_attachment_states, depth_stencil_state, models.pipeline_layout, offscreen.render_pass);
}
...
vk::RenderPass HPPHDR::create_render_pass(std::vector<vk::AttachmentDescription> const &attachment_descriptions,
...
void HPPHDR::prepare_offscreen_buffer()
{
...

	{
		...
		offscreen.framebuffer = vkb::common::create_framebuffer(get_device().get_handle(), offscreen.render_pass,
		                                                        {offscreen.color[0].view, offscreen.color[1].view, offscreen.depth.view}, offscreen.extent);

		// Create sampler to sample from the color attachments
		offscreen.sampler = vkb::common::create_sampler(get_device().get_gpu().get_handle(), get_device().get_handle(), color_format, vk::Filter::eNearest,
	...
	// Bloom separable filter pass
	{
		...
		filter_pass.sampler = vkb::common::create_sampler(get_device().get_gpu().get_handle(), get_device().get_handle(), color_format, vk::Filter::eNearest,
		                                                  vk::SamplerAddressMode::eClampToEdge, 1.0f, 1.0f);
	}
}
...
void HPPHDR::update_model_descriptor_set(vk::DescriptorSet descriptor_set)
{
	...
	vk::DescriptorImageInfo environment_image_descriptor{
	    textures.envmap.sampler, textures.envmap.image->get_vk_image_view().get_handle(),
