...
void onKeyEvent(int key, int scancode, int action, int mods) {
    ...
    if (ImGUIUtil::get_want_key_capture()) {
...
void onCharEvent(uint32_t codepoint) {
    ImGUIUtil::char_pressed(codepoint);
}
----
...
The `MapView` class holds only UI state — no GPU resources of its own:

[source,cpp]
----
class MapView {
public:
    enum class ViewState { Octomap, GLTFRegular, GLTFSplats };
...
To add button or panel textures, upload each image to a `VkImage` inside `initResources` (following the same pattern as the font texture upload), build a `VkWriteDescriptorSet`, and append it to `writeDescriptorSets` before calling `vkUpdateDescriptorSets`. The resulting `VkDescriptorSet` can then be passed as `ImDrawCmd::TextureId` and rendered with `ImGui::Image()` or `ImGui::ImageButton()`.
