                                                                         ...
                                                                         vkb::core::CommandBufferC                &main_command_buffer)
{
	...
	auto &shadow_command_buffer = get_render_context().get_active_frame().get_command_pool(queue, reset_mode, 1).request_command_buffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY);
                                                                           ...
                                                                           vkb::core::CommandBufferC                &main_command_buffer)
{
	...
	auto &scene_command_buffer = get_render_context().get_active_frame().get_command_pool(queue, reset_mode, 0).request_command_buffer(VK_COMMAND_BUFFER_LEVEL_SECONDARY);

	// Shadow pass will be recorded in thread with id 1
	auto &shadow_command_buffer = get_render_context().get_active_frame().get_command_pool(queue, reset_mode, 1).request_command_buffer(VK_COMMAND_BUFFER_LEVEL_SECONDARY);
