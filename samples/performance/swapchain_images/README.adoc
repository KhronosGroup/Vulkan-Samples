...
The application can ask for a minimum number of images by setting the `minImageCount` parameter in https://www.khronos.org/registry/vulkan/specs/latest/man/html/vkCreateSwapchainKHR.html[vkCreateSwapchainKHR].
The exact number of images created can then be polled via https://www.khronos.org/registry/vulkan/specs/latest/man/html/vkGetSwapchainImagesKHR.html[vkGetSwapchainImagesKHR].

In order to properly set the `minImageCount` parameter, the application should get the surface capabilities of the physical device via https://www.khronos.org/registry/vulkan/specs/latest/man/html/vkGetPhysicalDeviceSurfaceCapabilitiesKHR.html[vkGetPhysicalDeviceSurfaceCapabilitiesKHR].
The https://www.khronos.org/registry/vulkan/specs/latest/man/html/VkSurfaceCapabilitiesKHR.html[VkSurfaceCapabilitiesKHR] structure has the `minImageCount` and `maxImageCount` parameters, which set the boundaries for the image count that can be safely requested.
...
The available present modes can be queried via https://www.khronos.org/registry/vulkan/specs/latest/man/html/vkGetPhysicalDeviceSurfacePresentModesKHR.html[vkGetPhysicalDeviceSurfacePresentModesKHR].
...
* Ensure that the value of `minImageCount` is within the valid range from https://www.khronos.org/registry/vulkan/specs/latest/man/html/vkGetPhysicalDeviceSurfaceCapabilitiesKHR.html[vkGetPhysicalDeviceSurfaceCapabilitiesKHR] (between `minImageCount` and `maxImageCount`).
...
* It is possible to check how many images are created via https://www.khronos.org/registry/vulkan/specs/latest/man/html/vkGetSwapchainImagesKHR.html[vkGetSwapchainImagesKHR].
