...
MultiDrawIndirect::~MultiDrawIndirect()
{
	if (device)
	{
		...
		vkDestroySampler(device->get_handle(), sampler_linear, VK_NULL_HANDLE);
		vkDestroySampler(device->get_handle(), sampler_nearest, VK_NULL_HANDLE);
...
void MultiDrawIndirect::load_scene()
{
	...
	for (auto &&mesh : scene->get_components<vkb::sg::Mesh>())
	{
		...
		auto data_buffer = vkb::core::Buffer::create_staging_buffer(*device, image->get_data());
