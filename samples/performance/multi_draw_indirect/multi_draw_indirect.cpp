...
MultiDrawIndirect::~MultiDrawIndirect()
{
	if (has_device())
	{
		...
		vkDestroyDescriptorSetLayout(get_device().get_handle(), gpu_cull_descriptor_set_layout, VK_NULL_HANDLE);

		vkDestroyPipeline(get_device().get_handle(), device_address_pipeline, VK_NULL_HANDLE);
		vkDestroyPipelineLayout(get_device().get_handle(), device_address_pipeline_layout, VK_NULL_HANDLE);
		vkDestroyDescriptorSetLayout(get_device().get_handle(), device_address_descriptor_set_layout, VK_NULL_HANDLE);
	...
	for (size_t i = 0; i < draw_cmd_buffers.size(); ++i)
	{
		...
		if (m_enable_mdi && m_supports_mdi)
		{
			vkCmdDrawIndexedIndirect(
			    draw_cmd_buffers[i], indirect_call_buffer->get_handle(), 0, static_cast<uint32_t>(cpu_commands.size()), sizeof(cpu_commands[0]));
		}
		else
		{
...
bool MultiDrawIndirect::prepare(const vkb::ApplicationOptions &options)
{
	...
	if (!compute_queue)
	{
		compute_queue = &get_device().get_queue_by_flags(VK_QUEUE_COMPUTE_BIT, 0);
	}

	queue_families.clear();
	for (auto &&queue_bit : {VK_QUEUE_GRAPHICS_BIT, VK_QUEUE_COMPUTE_BIT})
	{
		const auto index = get_device().get_queue_by_flags(queue_bit, 0).get_family_index();
	...
	for (auto &&mesh : get_scene().get_components<vkb::sg::Mesh>())
	{
		...
		for (auto &&sub_mesh : mesh->get_submeshes())
		{
			...
			if (buffer)
			{
				...
				{
					model.triangles[i] = {static_cast<uint16_t>(temp_buffer[3 * i]),
...
void MultiDrawIndirect::initialize_resources()
{
	...
	indirect_call_buffer = std::make_unique<vkb::core::BufferC>(get_device(),
	...
	if (m_supports_buffer_device)
	{
		...
		VkDeviceAddress srcPtr = vkGetBufferDeviceAddressKHR(get_device().get_handle(), &address_info);
	...
	auto copy = [this, &cmd](vkb::core::BufferC &staging_buffer, VkBufferUsageFlags buffer_usage_flags) {
		auto output_buffer = std::make_unique<vkb::core::BufferC>(get_device(),
		                                                          staging_buffer.get_size(),
...
void MultiDrawIndirect::create_pipeline()
{
	...
	// Create descriptors
	auto create_descriptors = [this](const std::vector<VkDescriptorSetLayoutBinding> &set_layout_bindings,
	...
	std::vector<VkDescriptorSetLayoutBinding> set_layout_bindings = {
	    model_information_binding, image_array_binding, scene_uniform_binding, command_buffer_binding};
...
void MultiDrawIndirect::update_scene_uniform()
{
	...
	{
		scene_uniform_buffer = std::make_unique<vkb::core::BufferC>(get_device(),
		                                                            sizeof(SceneUniform),
...
void MultiDrawIndirect::run_gpu_cull()
{
	...
	auto                     cmd   = get_device().create_command_buffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY);
	...
	vkFreeCommandBuffers(get_device().get_handle(), get_device().get_command_pool().get_handle(), 1, &cmd);
}
...
void MultiDrawIndirect::cpu_cull()
{
	...
	{
		cpu_staging_buffer = std::make_unique<vkb::core::BufferC>(get_device(),
		                                                          models.size() * sizeof(VkDrawIndexedIndirectCommand),
