...
void CommandBufferUsage::draw_renderpass(vkb::core::CommandBufferC &primary_command_buffer, vkb::RenderTarget &render_target)
{
	...
	if (has_gui())
	{
		if (use_secondary_command_buffers)
		{
			...
			auto &secondary_command_buffer = get_render_context().get_active_frame().get_command_pool(queue, subpass->get_state().command_buffer_reset_mode).request_command_buffer(VK_COMMAND_BUFFER_LEVEL_SECONDARY);
                                                                       ...
                                                                       size_t                                                             thread_index)
{
	...
	auto &secondary_command_buffer = get_render_context().get_active_frame().get_command_pool(queue, state.command_buffer_reset_mode, thread_index).request_command_buffer(VK_COMMAND_BUFFER_LEVEL_SECONDARY);
...
void CommandBufferUsage::ForwardSubpassSecondary::draw(vkb::core::CommandBufferC &primary_command_buffer)
{
	...
	std::vector<vkb::core::CommandBufferC *> secondary_command_buffers;
	...
	if (use_secondary_command_buffers)
	{
		std::vector<std::future<vkb::core::CommandBufferC *>> secondary_cmd_buf_futures;
