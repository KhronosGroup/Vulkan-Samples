...
#include "my_sample_api.h"

MySampleApi::MySampleApi()
{
}
...
bool MySampleApi::prepare(const vkb::ApplicationOptions &options)
{
...
void MySampleApi::prepare_pipelines()
{
...
void MySampleApi::build_command_buffers()
{
...
void MySampleApi::render(float delta_time)
{
	if (!prepared)
	{
		return;
...
std::unique_ptr<vkb::VulkanSample<vkb::BindingType::C>> create_my_sample_api()
{
	return std::make_unique<MySampleApi>();
}
