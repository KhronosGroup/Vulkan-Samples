...
void MemoryBudget::setup_descriptor_pool()
{
	...
	VkDescriptorPoolCreateInfo descriptor_pool_create_info =
	    vkb::initializers::descriptor_pool_create_info(vkb::to_u32(pool_sizes.size()), pool_sizes.data(), 2);
...
void MemoryBudget::setup_descriptor_set()
{
	...
	write_descriptor_sets = {
	    vkb::initializers::write_descriptor_set(
	        descriptor_sets.instanced_rocks, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, &buffer_descriptor),        // Binding 0 : Vertex shader uniform buffer
	    vkb::initializers::write_descriptor_set(
	        descriptor_sets.instanced_rocks, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, &image_descriptor)        // Binding 1 : Color map
	};
	...
	write_descriptor_sets = {
	    vkb::initializers::write_descriptor_set(
	        descriptor_sets.planet, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, &buffer_descriptor),        // Binding 0 : Vertex shader uniform buffer
	    vkb::initializers::write_descriptor_set(
	        descriptor_sets.planet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, &image_descriptor)        // Binding 1 : Color map
	};
	vkUpdateDescriptorSets(get_device().get_handle(), vkb::to_u32(write_descriptor_sets.size()), write_descriptor_sets.data(), 0, nullptr);
}
...
void MemoryBudget::prepare_pipelines()
{
	...
	binding_descriptions = {// Binding point 0: Mesh vertex layout description at per-vertex rate
...
void MemoryBudget::prepare_instance_data()
{
	...
	for (auto i = 0; i < MESH_DENSITY_HALF; i++)
	{
		...
		theta = 2.0f * glm::pi<float>() * uniform_dist(rnd_generator);
		instance_data[static_cast<uint32_t>(i + MESH_DENSITY_HALF)].pos =
		    glm::vec3(rho * cos(theta), uniform_dist(rnd_generator) * 0.5f - 0.25f, rho * sin(theta));
		instance_data[static_cast<uint32_t>(i + MESH_DENSITY_HALF)].rot = glm::vec3(
		    glm::pi<float>() * uniform_dist(rnd_generator), glm::pi<float>() * uniform_dist(rnd_generator), glm::pi<float>() * uniform_dist(rnd_generator));
	...
	instance_buffer.buffer = std::make_unique<vkb::core::BufferC>(
	    get_device(), instance_buffer.size, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VMA_MEMORY_USAGE_GPU_ONLY);
	...
	vkCmdCopyBuffer(copy_command, staging_buffer.get_handle(), instance_buffer.buffer->get_handle(), 1, &copy_region);
...
{
	uniform_buffers.scene = std::make_unique<vkb::core::BufferC>(get_device(), sizeof(ubo_vs), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU);
