...
void ConditionalRendering::build_command_buffers()
{
	...
	for (int32_t i = 0; i < draw_cmd_buffers.size(); ++i)
	{
		...
		for (auto &node : linear_scene_nodes)
		{
			...
			// Start a conditional rendering block, commands in this block are only executed if the buffer at the current position is 1 at command buffer
			// submission time
...
void ConditionalRendering::prepare_pipelines()
{
	...
	std::vector<VkPipelineColorBlendAttachmentState> blend_attachment_states = {vkb::initializers::pipeline_color_blend_attachment_state(0xf, VK_FALSE)};
