 ...
 *          to Images and Buffers. This is read as an input by a `Data Graph Pipeline` (`graph_pipeline`), which is a new type of pipeline analogous
 ...
 *
 *					input_tensor -> graph_pipeline -> output_tensor
 *                         \                                 \.
 ...
 *			generic functionality for these concepts has been added to a new graph_pipelines_common.h/cpp file, which this sample
 *			(and other graph pipeline samples) makes use of.
 */
class HelloGraphPipeline : public vkb::VulkanSampleC
{
  public:
	HelloGraphPipeline();
	~HelloGraphPipeline();
  ...
  private:
	...
	void prepare_graph_pipeline();
	void prepare_graph_pipeline_descriptor_set();
	...
	std::unique_ptr<GraphPipelineLayout>  graph_pipeline_layout;
	std::unique_ptr<GraphPipeline>        graph_pipeline;
	std::unique_ptr<GraphPipelineSession> graph_pipeline_session;
	VkDescriptorSet                       graph_pipeline_descriptor_set = VK_NULL_HANDLE;
