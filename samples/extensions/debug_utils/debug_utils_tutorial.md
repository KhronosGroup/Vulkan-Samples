...
This tutorial, along with the accompanying example code, demonstrates the use of the ```VK_EXT_debug_utils``` extension to setup a validation layer messenger callback and pass additional debugging information to debuggers like [RenderDoc](https://renderdoc.org/).
...
**Note**: Enabling the extension is done inside the framework, see the ```Instance``` class in ```instance.cpp``` for details.

Enabling the functionality for the debug utilities is done by adding the extension to the list of extension to enable at instance level. 
...
Imagine you need to debug a problem with a shader module not properly working or seemingly the wrong shader used by a pipeline. Without adding names to your Vulkan objects, all your resources will have similar names only distinguished by a random number:
...
Finding "Shader Module 257" or any of the "Graphics Pipeline 259/260" in your code could will prove tricky to impossible.

But if you're using the new extension to add meaningful names to your, connecting the resources and finding them in your application becomes straightforward:
