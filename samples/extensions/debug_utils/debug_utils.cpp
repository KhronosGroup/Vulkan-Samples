...
void DebugUtils::cmd_begin_label(VkCommandBuffer command_buffer, const char *label_name, std::vector<float> color) const
{
	if (!debug_utils_supported)
	{
		return;
...
void DebugUtils::cmd_insert_label(VkCommandBuffer command_buffer, const char *label_name, std::vector<float> color) const
{
	if (!debug_utils_supported)
	{
		return;
...
void DebugUtils::cmd_end_label(VkCommandBuffer command_buffer) const
{
	if (!debug_utils_supported)
	{
		return;
...
void DebugUtils::queue_begin_label(VkQueue queue, const char *label_name, std::vector<float> color) const
{
	if (!debug_utils_supported)
	{
		return;
...
void DebugUtils::queue_insert_label(VkQueue queue, const char *label_name, std::vector<float> color) const
{
	if (!debug_utils_supported)
	{
		return;
...
void DebugUtils::queue_end_label(VkQueue queue) const
{
	if (!debug_utils_supported)
	{
		return;
...
void DebugUtils::debug_name_objects()
{
	...
	set_object_name(VK_OBJECT_TYPE_PIPELINE, (uint64_t) pipelines.skysphere, "Sky sphere pipeline");
	set_object_name(VK_OBJECT_TYPE_PIPELINE, (uint64_t) pipelines.composition, "Sky sphere pipeline");
...
void DebugUtils::create_attachment(VkFormat format, VkImageUsageFlagBits usage, FrameBufferAttachment *attachment)
{
	...
	VK_CHECK(vkCreateImage(device->get_handle(), &image, nullptr, &attachment->image));
	vkGetImageMemoryRequirements(device->get_handle(), attachment->image, &memory_requirements);
	...
	VK_CHECK(vkAllocateMemory(device->get_handle(), &memory_allocate_info, nullptr, &attachment->mem));
...
{
	{
		...
		VK_CHECK(vkCreateRenderPass(device->get_handle(), &render_pass_create_info, nullptr, &offscreen.render_pass));
		...
		VK_CHECK(vkCreateFramebuffer(device->get_handle(), &framebuffer_create_info, nullptr, &offscreen.framebuffer));
		...
		VK_CHECK(vkCreateSampler(device->get_handle(), &sampler, nullptr, &offscreen.sampler));
	}
	...
	// Bloom separable filter pass
	{
		...
		VK_CHECK(vkCreateRenderPass(device->get_handle(), &render_pass_create_info, nullptr, &filter_pass.render_pass));
		...
		framebuffer_create_info.layers                  = 1;
		VK_CHECK(vkCreateFramebuffer(device->get_handle(), &framebuffer_create_info, nullptr, &filter_pass.framebuffer));
		...
		sampler.maxLod              = 1.0f;
		sampler.borderColor         = VK_BORDER_COLOR_FLOAT_OPAQUE_WHITE;
		VK_CHECK(vkCreateSampler(device->get_handle(), &sampler, nullptr, &filter_pass.sampler));
	}
}
...
void DebugUtils::setup_descriptor_set_layout()
{
	...
	VK_CHECK(vkCreateDescriptorSetLayout(device->get_handle(), &descriptor_layout_create_info, nullptr, &descriptor_set_layouts.models));
	...
	VK_CHECK(vkCreatePipelineLayout(device->get_handle(), &pipeline_layout_create_info, nullptr, &pipeline_layouts.models));
	...
	VK_CHECK(vkCreateDescriptorSetLayout(device->get_handle(), &descriptor_layout_create_info, nullptr, &descriptor_set_layouts.bloom_filter));

	pipeline_layout_create_info = vkb::initializers::pipeline_layout_create_info(&descriptor_set_layouts.bloom_filter, 1);
	VK_CHECK(vkCreatePipelineLayout(device->get_handle(), &pipeline_layout_create_info, nullptr, &pipeline_layouts.bloom_filter));
	...
	descriptor_layout_create_info = vkb::initializers::descriptor_set_layout_create_info(set_layout_bindings.data(), static_cast<uint32_t>(set_layout_bindings.size()));
	VK_CHECK(vkCreateDescriptorSetLayout(device->get_handle(), &descriptor_layout_create_info, nullptr, &descriptor_set_layouts.composition));

	pipeline_layout_create_info = vkb::initializers::pipeline_layout_create_info(&descriptor_set_layouts.composition, 1);
	VK_CHECK(vkCreatePipelineLayout(device->get_handle(), &pipeline_layout_create_info, nullptr, &pipeline_layouts.composition));
}
...
void DebugUtils::setup_descriptor_sets()
{
	...
	VK_CHECK(vkAllocateDescriptorSets(device->get_handle(), &alloc_info, &descriptor_sets.sphere));
	...
	VK_CHECK(vkAllocateDescriptorSets(device->get_handle(), &alloc_info, &descriptor_sets.skysphere));
	...
	VK_CHECK(vkAllocateDescriptorSets(device->get_handle(), &alloc_info, &descriptor_sets.bloom_filter));
	...
	VK_CHECK(vkAllocateDescriptorSets(device->get_handle(), &alloc_info, &descriptor_sets.composition));
...
void DebugUtils::prepare_pipelines()
{
	...
	VK_CHECK(vkCreateGraphicsPipelines(device->get_handle(), pipeline_cache, 1, &pipeline_create_info, nullptr, &pipelines.composition));
	...
	VK_CHECK(vkCreateGraphicsPipelines(device->get_handle(), pipeline_cache, 1, &pipeline_create_info, nullptr, &pipelines.bloom[0]));

	// Second blur pass (into separate framebuffer)
	pipeline_create_info.renderPass = filter_pass.render_pass;
	dir                             = 0;
	VK_CHECK(vkCreateGraphicsPipelines(device->get_handle(), pipeline_cache, 1, &pipeline_create_info, nullptr, &pipelines.bloom[1]));
	...
	// sky sphere pipeline (background cube)
...
void DebugUtils::on_update_ui_overlay(vkb::Drawer &drawer)
{
	if (debug_utils_supported)
	{
		vkb::Drawer::text("Debug utilities enabled");
	}
	else
	{
		vkb::Drawer::text("Warning: Debug utilities extension not available");
		vkb::Drawer::text("Possible reasons:");
		vkb::Drawer::text("- Driver does not support the extension");
		vkb::Drawer::text("- Compiling in release mode with no validation layers enabled");
	}
