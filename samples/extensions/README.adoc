...
=== xref:./{extension_samplespath}graph_pipelines/README.adoc[Graph Pipelines]

*Extensions*: `VK_ARM_tensors`, `VK_ARM_data_graph`

There are a series of graph pipeline based examples each focusing on a specific aspect of building and executing a neural graph pipeline using Vulkan:

1. xref:./{extension_samplespath}graph_pipelines/hello_graph_pipeline/README.adoc[hello_graph_pipeline]
* Explains how to set up and execute a simple neural network using a graph pipeline.
