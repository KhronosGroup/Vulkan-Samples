...
* Aliased as a regular `VkImage` and used as a texture to be sampled from in another shader. See the "Tensor image aliasing" sample xref:samples/extensions/tensor_and_data_graph/tensor_image_aliasing/README.adoc[README].
...
* Aliased as a regular `VkImage` and written to via a render target (or writable texture).  See the "Tensor image aliasing" sample xref:samples/extensions/tensor_and_data_graph/tensor_image_aliasing/README.adoc[README].
