...
void DynamicMultisampleRasterization::prepare_supported_sample_count_list()
{
	...
	std::copy_if(counts.begin(), counts.end(), std::back_inserter(supported_sample_count_list), [&supported_by_depth_and_color](auto count) {
		return supported_by_depth_and_color & count;
	});
	std::transform(supported_sample_count_list.begin(), supported_sample_count_list.end(), std::back_inserter(gui_settings.sample_counts), [](auto count) {
