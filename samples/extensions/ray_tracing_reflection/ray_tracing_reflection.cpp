...
struct ObjCube : ObjModelCpu
{
	ObjCube()
	{
		vertices = {
		    ...
		    {{-0.5f, +0.5f, +0.5f}, {+0.f, +1.f, +0.f}}, {{+0.5f, +0.5f, -0.5f}, {+0.f, +1.f, +0.f}}, {{-0.5f, +0.5f, -0.5f}, {+0.f, +1.f, +0.f}},
		    {{+0.5f, -0.5f, +0.5f}, {+0.f, -1.f, +0.f}},        // Bottom
		    {{-0.5f, -0.5f, +0.5f}, {+0.f, -1.f, +0.f}}, {{+0.5f, -0.5f, -0.5f}, {+0.f, -1.f, +0.f}}, {{-0.5f, -0.5f, -0.5f}, {+0.f, -1.f, +0.f}},
		    {{+0.5f, +0.5f, +0.5f}, {+1.f, +0.f, +0.f}},        // Right
		    {{+0.5f, +0.5f, -0.5f}, {+1.f, +0.f, +0.f}}, {{+0.5f, -0.5f, -0.5f}, {+1.f, +0.f, +0.f}}, {{+0.5f, -0.5f, +0.5f}, {+1.f, +0.f, +0.f}},
		    {{-0.5f, +0.5f, +0.5f}, {-1.f, +0.f, +0.f}},        // left
		    {{-0.5f, +0.5f, -0.5f}, {-1.f, +0.f, +0.f}}, {{-0.5f, -0.5f, -0.5f}, {-1.f, +0.f, +0.f}}, {{-0.5f, -0.5f, +0.5f}, {-1.f, +0.f, +0.f}},
		    {{-0.5f, +0.5f, +0.5f}, {+0.f, +0.f, +1.f}},        // front
		    {{+0.5f, +0.5f, +0.5f}, {+0.f, +0.f, +1.f}}, {{+0.5f, -0.5f, +0.5f}, {+0.f, +0.f, +1.f}}, {{-0.5f, -0.5f, +0.5f}, {+0.f, +0.f, +1.f}},
...
void RaytracingReflection::create_bottom_level_acceleration_structure(ObjModelGpu &obj_model)
{
	...
	blas.buffer = std::make_unique<vkb::core::BufferC>(get_device(), acceleration_structure_build_sizes_info.accelerationStructureSize,
	...
	sc_buffer = std::make_unique<vkb::core::BufferC>(get_device(),
...
void RaytracingReflection::create_top_level_acceleration_structure(std::vector<VkAccelerationStructureInstanceKHR> &blas_instances)
{
	std::unique_ptr<vkb::core::BufferC> instances_buffer = std::make_unique<vkb::core::BufferC>(
	    get_device(), sizeof(VkAccelerationStructureInstanceKHR) * blas_instances.size(),
...
void RaytracingReflection::create_shader_binding_tables()
{
	...
	raygen_shader_binding_table =
	    std::make_unique<vkb::core::BufferC>(get_device(), vkb::core::BufferBuilderC(handle_size_aligned * rgen_index.size())
	...
	miss_shader_binding_table =
	    std::make_unique<vkb::core::BufferC>(get_device(), vkb::core::BufferBuilderC(handle_size_aligned * miss_index.size())
	                                                           .with_usage(sbt_buffer_usage_flags)
	                                                           .with_vma_usage(sbt_memory_usage)
	                                                           .with_alignment(ray_tracing_pipeline_properties.shaderGroupBaseAlignment));
	hit_shader_binding_table =
	    std::make_unique<vkb::core::BufferC>(get_device(), vkb::core::BufferBuilderC(handle_size_aligned * hit_index.size())
	                                                           .with_usage(sbt_buffer_usage_flags)
	                                                           .with_vma_usage(sbt_memory_usage)
	                                                           .with_alignment(ray_tracing_pipeline_properties.shaderGroupBaseAlignment));
...
void RaytracingReflection::create_descriptor_sets()
{
	...
	VkWriteDescriptorSet uniform_buffer_write =
	    vkb::initializers::write_descriptor_set(descriptor_set, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 2, &uniform_descriptor);
...
void RaytracingReflection::create_ray_tracing_pipeline()
{
	...
	*/
	...
	{
		shader_stages.push_back(load_shader("ray_tracing_reflection/raygen.rgen.spv", VK_SHADER_STAGE_RAYGEN_BIT_KHR));
	...
	// Ray miss group
	{
		shader_stages.push_back(load_shader("ray_tracing_reflection/miss.rmiss.spv", VK_SHADER_STAGE_MISS_BIT_KHR));
	...
	{
		shader_stages.push_back(load_shader("ray_tracing_reflection/missShadow.rmiss.spv", VK_SHADER_STAGE_MISS_BIT_KHR));
		VkRayTracingShaderGroupCreateInfoKHR miss_group_ci{VK_STRUCTURE_TYPE_RAY_TRACING_SHADER_GROUP_CREATE_INFO_KHR};
		miss_group_ci.type               = VK_RAY_TRACING_SHADER_GROUP_TYPE_GENERAL_KHR;
		miss_group_ci.generalShader      = static_cast<uint32_t>(shader_stages.size()) - 1;
		miss_group_ci.closestHitShader   = VK_SHADER_UNUSED_KHR;
