...
void RayTracingPositionFetch::create_bottom_level_acceleration_structure()
{
	...
	for (auto &&mesh : scene->get_components<vkb::sg::Mesh>())
	{
		for (auto &&sub_mesh : mesh->get_submeshes())
		{
			...
			bottom_level_acceleration_structure->add_triangle_geometry(sub_mesh->vertex_buffers.at("position"),
...
void RayTracingPositionFetch::create_ray_tracing_pipeline()
{
	std::vector<VkDescriptorSetLayoutBinding> bindings = {
	    ...
	    vkb::initializers::descriptor_set_layout_binding(
	        VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR, 2, 1)};
