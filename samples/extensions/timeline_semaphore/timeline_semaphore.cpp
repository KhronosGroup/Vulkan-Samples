...
void TimelineSemaphore::setup_shared_resources()
{
	...
	// Descriptor layouts
	{
		...
		VkDescriptorSetLayoutBinding sampled_binding =
		    vkb::initializers::descriptor_set_layout_binding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_ALL, 0);
	...
	// Descriptor sets
	{
		...
		for (int i = 0; i < NumAsyncFrames; ++i)
		{
			...
			auto readonly_info =
			    vkb::initializers::descriptor_image_info(VK_NULL_HANDLE, shared.image_views[i]->get_handle(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
...
void TimelineSemaphore::setup_compute_resources()
{
	...
	compute.command_pool =
	    get_device().create_command_pool(compute.queue_family_index, VK_COMMAND_POOL_CREATE_TRANSIENT_BIT | VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT);

	VkCommandBufferAllocateInfo alloc_info = vkb::initializers::command_buffer_allocate_info(compute.command_pool, VK_COMMAND_BUFFER_LEVEL_PRIMARY, 1);
...
void TimelineSemaphore::build_compute_command_buffers(const float elapsed)
{
	...
	else
	{
		...
		vkCmdPushConstants(compute.command_buffer, compute.pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(elapsed), &elapsed);
	}
...
void TimelineSemaphore::setup_graphics_resources()
{
	...
	graphics.command_pool =
	    get_device().create_command_pool(graphics.queue_family_index, VK_COMMAND_POOL_CREATE_TRANSIENT_BIT | VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT);

	VkCommandBufferAllocateInfo alloc_info = vkb::initializers::command_buffer_allocate_info(graphics.command_pool, VK_COMMAND_BUFFER_LEVEL_PRIMARY, 1);
