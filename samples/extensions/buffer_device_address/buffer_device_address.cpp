...
BufferDeviceAddress::TestBuffer BufferDeviceAddress::create_vbo_buffer()
{
	...
	VkBufferCreateInfo create_info =
	    vkb::initializers::buffer_create_info(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT_KHR, mesh_size);
...
void BufferDeviceAddress::update_pointer_buffer(VkCommandBuffer cmd)
{
	...
	vkCmdPipelineBarrier(cmd, VK_PIPELINE_STAGE_VERTEX_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, 0, nullptr);
...
void BufferDeviceAddress::update_meshlets(VkCommandBuffer cmd)
{
	...
	vkCmdPushConstants(cmd, pipelines.compute_pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(push_compute), &push_compute);
	...
	global_memory_barrier.dstAccessMask   = VK_ACCESS_SHADER_READ_BIT;
	vkCmdPipelineBarrier(cmd, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_VERTEX_SHADER_BIT, 0, 1, &global_memory_barrier, 0, nullptr, 0, nullptr);
}
...
void BufferDeviceAddress::render(float delta_time)
{
	...
	push_vertex.view_projection = glm::perspective(0.5f * glm::pi<float>(), static_cast<float>(width) / static_cast<float>(height), 1.0f, 100.0f) *
