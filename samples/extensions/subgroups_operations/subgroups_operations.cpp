...
{
	grid_size = static_cast<uint32_t>(stoi(ui.grid_sizes[ui.grid_size_idx]));
	...
	for (uint32_t z = 0; z <= grid_size; ++z)
	{
		...
		{
			Vertex point = {};
			point.pos.x  = static_cast<float>(x) / static_cast<float>(grid_size);
			point.pos.z  = static_cast<float>(z) / static_cast<float>(grid_size);
			point.pos.y  = 0.0f;
	...
	for (uint32_t z = 0; z < grid_size; ++z)
	{
		for (uint32_t x = 0; x < grid_size; ++x)
		{
			uint32_t i0 = z * (grid_size + 1) + x;
			uint32_t i1 = i0 + 1;
			uint32_t i2 = i0 + (grid_size + 1);
			uint32_t i3 = i2 + 1;
	...
	if (input_grid.vertex)
	{
		input_grid.vertex.reset(nullptr);
	}
	if (ocean.grid.vertex && ocean.grid.index)
	{
		ocean.grid.vertex.reset(nullptr);
		ocean.grid.index.reset(nullptr);
	}
...
void SubgroupsOperations::on_update_ui_overlay(vkb::Drawer &drawer)
{
	if (drawer.header("Settings"))
	{
		...
		if (drawer.combo_box("Grid Size", &ui.grid_size_idx, ui.grid_sizes))
		{
			// recreate grid mesh
			generate_grid();
			update_uniform_buffers();
