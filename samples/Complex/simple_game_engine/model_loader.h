...
public:
    ...
    void SetAnimations(const std::vector<Animation>& anims) { animations = anims; }
    ...
    [[nodiscard]] const std::unordered_map<int, glm::mat4>& GetAnimatedNodeTransforms() const { return animatedNodeTransforms; }
    void SetAnimatedNodeTransforms(const std::unordered_map<int, glm::mat4>& transforms) { animatedNodeTransforms = transforms; }

    // Animated node to mesh mapping: maps glTF node index to mesh index
    // Used to link animated nodes to their geometry entities
    [[nodiscard]] const std::unordered_map<int, int>& GetAnimatedNodeMeshes() const { return animatedNodeMeshes; }
    void SetAnimatedNodeMeshes(const std::unordered_map<int, int>& meshes) { animatedNodeMeshes = meshes; }
