...
void Renderer::renderReflectionPass(vk::raii::CommandBuffer& cmd,
    ...
    if (reflections.empty()) return;
    auto &rt = reflections[currentFrame];
    if (rt.width == 0 || rt.height == 0 || rt.colorView == nullptr || rt.depthView == nullptr) return;
    ...
    for (const auto& uptr : entities) {
        ...
        if (auto tc = entity->GetComponent<TransformComponent>()) ubo.model = tc->GetModelMatrix(); else ubo.model = glm::mat4(1.0f);
...
void Renderer::Render(const std::vector<std::unique_ptr<Entity>>& entities, CameraComponent* camera, ImGuiSystem* imguiSystem) {
        ...
        size_t readyUniqueMeshCount = 0;
        {
            ...
            for (const auto& uptr : entities) {
                ...
                if (IsRayQueryStaticOnly()) {
                    ...
                    if (nm.find("_AnimNode_") != std::string::npos) continue;
                    if (!nm.empty() && nm.rfind("Ball_", 0) == 0) continue;
                }
                ...
                try {
                    ...
                    if (res.vertexBufferSizeBytes != 0 || res.indexBufferSizeBytes != 0) continue;
                    if (!*res.vertexBuffer || !*res.indexBuffer) continue;
                    if (res.indexCount == 0) continue;
                } catch (...) {
                    continue;
                }
    ...
    if (asBuildRequested.load(std::memory_order_acquire)) {
        ...
        if (asFrozen && !asDevOverrideAllowRebuild) {
        ...
        const double readiness = (totalRenderableEntities > 0)
                                   ? static_cast<double>(readyRenderableCount) / static_cast<double>(totalRenderableEntities)
                                   : 0.0;
        ...
        if (!staticLights.empty()) {
            ...
            for (const auto& L : staticLights) {
                if (L.type == ExtractedLight::Type::Emissive) {
                    ++emissiveCount;
                    if (emissiveCount >= MAX_ACTIVE_LIGHTS) break;
                }
            }
    ...
    if (currentRenderMode == RenderMode::RayQuery && rayQueryEnabled && accelerationStructureEnabled) {
        ...
        if (!*tlasStructure.handle) {
            ...
            if (imageIndex < swapChainImageLayouts.size()) swapChainImageLayouts[imageIndex] = swapchainBarrier.newLayout;
            ...
            if (imageIndex < swapChainImageLayouts.size()) swapChainImageLayouts[imageIndex] = swapchainToColor.newLayout;
            ...
            struct CompositePush { float exposure; float gamma; int outputIsSRGB; float _pad; } pc2{};
            ...
            if (imageIndex < swapChainImageLayouts.size()) swapChainImageLayouts[imageIndex] = swapchainToPresent.newLayout;
    ...
    if (useForwardPlus) {
        ...
        glm::mat4 proj = camera->GetProjectionMatrix(); proj[1][1] *= -1.0f;
    ...
    // PASS 1b: PRESENT – composite path
    {
            ...
            vk::DescriptorSet setComposite = transparentDescriptorSets.empty()
            ...
            // Push exposure/gamma and sRGB flag
            struct CompositePush { float exposure; float gamma; int outputIsSRGB; float _pad; } pc{};
...

    {
        ...
        // Final layout transition for present (rasterization path only)
        {
            ...
            if (imageIndex < swapChainImageLayouts.size()) swapChainImageLayouts[imageIndex] = presentBarrier2.newLayout;
        }
    }
    ...
    if (imguiSystem) {
        ...
        if (imageIndex < swapChainImageLayouts.size()) swapChainImageLayouts[imageIndex] = presentToColor.newLayout;
        ...
        if (imageIndex < swapChainImageLayouts.size()) swapChainImageLayouts[imageIndex] = colorToPresent.newLayout;
    }
    ...
    if (framebufferResized.load(std::memory_order_relaxed)) {
        ...
        { std::lock_guard<std::mutex> lock(queueMutex); graphicsQueue.submit2(emptySubmit2, *inFlightFences[currentFrame]); }
        recreateSwapChain();
        return;
    }
    ...
    { std::lock_guard<std::mutex> lock(queueMutex); graphicsQueue.submit2(submit2, *inFlightFences[currentFrame]); }
