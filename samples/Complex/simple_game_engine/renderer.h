...
public:
    ...
    void SetReflectionIntensity(float v) { reflectionIntensity = v; }
    float GetReflectionIntensity() const { return reflectionIntensity; }

    void SetPlanarReflectionsEnabled(bool enabled);
    void TogglePlanarReflections();
    bool IsPlanarReflectionsEnabled() const { return enablePlanarReflections; }

    // Ray query rendering mode control
    void SetRenderMode(RenderMode mode) { currentRenderMode = mode; }
    RenderMode GetRenderMode() const { return currentRenderMode; }
    void ToggleRenderMode() { 
        currentRenderMode = (currentRenderMode == RenderMode::Rasterization) 
            ? RenderMode::RayQuery 
            : RenderMode::Rasterization; 
    }
    
    // Ray query capability getters
    bool GetRayQueryEnabled() const { return rayQueryEnabled; }
    bool GetAccelerationStructureEnabled() const { return accelerationStructureEnabled; }

    // Ray Query static-only mode (disable animation/physics updates and TLAS refits to render a static opaque scene)
    void SetRayQueryStaticOnly(bool v) { rayQueryStaticOnly = v; }
    bool IsRayQueryStaticOnly() const { return rayQueryStaticOnly; }
     ...
     */
    void RequestAccelerationStructureBuild() { asBuildRequested.store(true, std::memory_order_release); }
    // Overload with reason tracking for diagnostics
    void RequestAccelerationStructureBuild(const char* reason) {
        if (reason) lastASBuildRequestReason = reason; else lastASBuildRequestReason = "(no reason)";
...
private:
    ...
    // Upload perf getters
public:
    uint64_t GetBytesUploadedTotal() const { return bytesUploadedTotal.load(std::memory_order_relaxed); }
