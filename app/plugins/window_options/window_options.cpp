...
void WindowOptions::init(const vkb::CommandParser &parser)
{
	...
	{
		properties.extent.width = parser.as<uint32_t>(&width_flag);
	}

	if (parser.contains(&height_flag))
	{
		properties.extent.height = parser.as<uint32_t>(&height_flag);
	}
	...
	platform->set_default_window_properties(properties);
}
}        // namespace plugins