...
TEST_CASE("load png with ktx2", "[assets]")
{
	...
	CHECK(fs.file_exists(TEST_IMAGE_2));

	std::shared_ptr<vfs::Blob> blob;

	auto err = fs.read_file(TEST_IMAGE_2, &blob);
	...
	// byte offset decreasing as KTX2 stores mips in memory lowest to highest
	for (size_t i = 0; i < image->mips.size(); i++)
	{
		uint32_t byte_length = image_width * image_width * 4;
		image_width          = image_width / 2;

		CHECK(image->mips[i].offset == image->data.size() - previous_offset - byte_length);
		previous_offset += byte_length;
