...
StackErrorPtr AstcCodec::decode(const Image &image, ImagePtr *o_image) const
{
	...
	{
		return StackError::unique("input format not supported by this codec", "images/astc.cpp", __LINE__);
	}
	...
	ImagePtr output_image = std::shared_ptr<images::Image>();
	...
	for (const auto &mip : image.mips)
	{
		...
		if (status != ASTCENC_SUCCESS)
		{
			return StackError::unique("ASTC Codec decode failed: " + std::string{astcenc_get_error_string(status)}, "images/astc.cpp", __LINE__);
		}
