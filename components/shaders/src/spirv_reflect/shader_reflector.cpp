...
inline std::string get_type_name(const SpvReflectTypeDescription &description)
{
	if (description.type_name != nullptr && strlen(description.type_name) > 0)
	{
	...
	if ((description.type_flags & SPV_REFLECT_TYPE_FLAG_FLOAT) == SPV_REFLECT_TYPE_FLAG_FLOAT)
	{
		ss << "float";
		return ss.str();
	}

	if ((description.type_flags & SPV_REFLECT_TYPE_FLAG_INT) == SPV_REFLECT_TYPE_FLAG_INT)
	{
		if (!!description.traits.numeric.scalar.signedness)
		{
			ss << "uint" << description.traits.numeric.scalar.width << "_t";
			return ss.str();
		}

		ss << "int" << description.traits.numeric.scalar.width << "_t";
		return ss.str();
	}

	if ((description.type_flags & SPV_REFLECT_TYPE_FLAG_BOOL) == SPV_REFLECT_TYPE_FLAG_BOOL)
	{
		ss << "bool";
		return ss.str();
	}
...
std::shared_ptr<RawShaderData> process_resource_type(const SpvReflectTypeDescription &description);

std::shared_ptr<RawShaderData> process_numeric_type(const SpvReflectTypeDescription &description)
{
	auto data = std::make_shared<NumericShaderData>();
	...
	data->type_name          = detail::get_type_name(description);
...
std::shared_ptr<RawShaderData> process_vector_type(const SpvReflectTypeDescription &description)
{
	auto data                = std::make_shared<VectorShaderData>();
...
std::shared_ptr<RawShaderData> process_matrix_type(const SpvReflectTypeDescription &description)
{
	auto data                = std::make_shared<MatrixShaderData>();
	data->type               = ShaderRawDataType::Matrix;
	data->type_name          = detail::get_type_name(description);
	data->struct_member_name = description.struct_member_name ? description.struct_member_name : "";
	data->columns            = description.traits.numeric.matrix.column_count;
	data->rows               = description.traits.numeric.matrix.row_count;
	return data;
}

std::shared_ptr<RawShaderData> process_struct_type(const SpvReflectTypeDescription &description)
{
	auto data                = std::make_shared<StructShaderData>();
...
std::shared_ptr<RawShaderData> process_array_type(const SpvReflectTypeDescription &description)
{
	...
	auto data                = std::make_shared<ArrayShaderData>();
	data->type               = ShaderRawDataType::Array;
	data->type_name          = detail::get_type_name(description);
	data->struct_member_name = description.struct_member_name ? description.struct_member_name : "";
	data->element_count      = description.traits.array.dims[0];

	if (description.member_count == 0)
	{
		LOGI("Array has no members, skipping");
		return RawShaderData::create_unknown();
	}

	if (description.member_count == 1)
	{
		data->element_type = process_resource_type(description.members[0]);
		return data;
	}

	if (strlen(description.members[0].struct_member_name) == 0)
	{
		// this is not a struct array
		return RawShaderData::create_unknown();
	}

	// the array can be described as a struct
	data->element_type = process_struct_type(description);
...
ShaderResources SpirvReflectShaderReflector::reflect_spirv(const std::vector<uint32_t> &spirv) const
{
	...
	std::vector<shaders::ShaderResource>   resources;
	...
	for (auto *variable : variables)
	{
		...
		resources.push_back(process_resource(*variable));
		spirv_id_to_resource_index[variable->spirv_id] = static_cast<uint32_t>(resources.size() - 1);
	}
